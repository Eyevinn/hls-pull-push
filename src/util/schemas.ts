...
export const Schemas = (name: string, plugins: IOutputPluginType[]) => {
  ...
  if (name === 'POST/fetcher') {
    return {
      description:
        'Creates and initializes a fetcher that pulls HLS live stream and pushes to a destination',
      tags: ['fetcher'],
      body: {
        type: 'object',
        properties: {
          ...
          url: {
            ...
            example:
              'https://demo.vc.eyevinn.technology/channels/eyevinn/master.m3u8'
          },
          output: {
            type: 'string',
          ...
          concurrency: {
            type: 'number',
            description:
              'Max number of concurrent jobs for fetcher. Default=10',
            example: 10
          },
          windowSize: {
            type: 'string',
            description:
              'Desired max window size (seconds) of the uploaded media playlists. Default=120',
            example: 120
          }
        },
      ...
      response: {
        200: {
          ...
          example: {
            message:
              'Created an Fetcher and started pulling from HLS Live Stream',
        ...
        200: {
          ...
          items: {
            ...
            properties: {
              fetcherId: {
                type: 'string',
                description: 'Unique identifier for fetcher session'
              },
              ...
              concurrency: {
                type: 'number',
                description: 'Number of maximum concurrent workers'
              },
              windowSize: {
                type: 'number',
                description:
                  'Number is seconds, set output media playlist window size'
              },
              sourcePlaylistType: {
                type: 'string',
                description:
                  'Source HLS Playlist Type, VOD | LIVE | EVENT | NONE'
              }
            },
            example: {
    ...
    return {
      ...
      response: {
        ...
        404: BadRequestSchema(
          "Fetcher with ID: '886008d7-e53a-4cab-a108-323b88352afd' was not found"
        )
      }
    };
  }
